...
var Orange = {
	...
	callbacks: {
		storage_get: function() {
			...
			$.each(Orange.queries, function(query, display_query) {
				if (query == "hn") {
					query_list += "<li><a href='#' class='close'>x</a><a href='#' data-search='hn'>news.ycombinator.com</a></li>";
				} else {
					query_list += "<li><a href='#' class='close'>x</a><a href='#' data-search=" + query + ">" + display_query + "</a></li>";
				}
			});

			$("nav ul.searches").append(query_list).hide().fadeIn(350);				
		},
	...
	hnsearch: {
	  ...
	  parse_json: function(results, url, query, start, append) {    
	      ...
	      article = {
			...
			var articles_string = "",
					infinite_scroller = "<span class='infinite-scroll pre-render'><a href='#'>More Submissions</a></span>";
  ...
  spinner: {
    ...
    show: function() {
			...
			window.spinner = new Spinner(Orange.spinner.opts).spin(target);
    },
    hide: function() {
      window.spinner.stop();
    }
	...
	storage: {
	  ...
	  destroy: function() {
	    delete localStorage["orange_queries"];
	  }
	},
